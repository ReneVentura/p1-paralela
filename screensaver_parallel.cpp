...
*/
void ballCollisionManager(Star &star1, Star &star2) {
    int dx = star1.x - star2.x;
    int dy = star1.y - star2.y;
    float distance;
    float overlap;
    ...
    overlap = star1.radius + star2.radius - distance;
    nx = dx / distance;
    ny = dy / distance;
    ...
    relativeVelocityX = star1.dx - star2.dx;
    relativeVelocityY = star1.dy - star2.dy;
    impulse = (relativeVelocityX * nx + relativeVelocityY * ny) * elasticity;
...
//Inicia el array de las bolas, agrega el contador de frames igual que su avg, inicia al window y el renderer. Pide cuantas bolas se quieren
int main(int argc, char* argv[]) {
   ...
   int max_threads= 16;
    ...
    // array para la posicion y velocidad de cada estrella
...
while (!quit) {
    ...
    for ( int i = 0; i < num_stars; i++) {
        ...
        // revisa si las estrellas colisionan con las paredes
        ...
        if (stars[i].bounces > 10 || stars[i].radius <= 0) {
           ...
           #pragma omp parallel for
            ...
            Star new_stars;
            new_stars.x = rand() % (windowWidth - starRadius * 2) + starRadius;
            new_stars.y = starRadius;
            new_stars.dx = rand() % 400 - 200;
            new_stars.dy = rand() % 400 - 200;
            new_stars.bounces = 0;
            new_stars.radius = starRadius;
            new_stars.r = rand() % 256;
            new_stars.g = rand() % 256;
            new_stars.b = rand() % 256;
            stars[num_stars] = new_stars;
            num_stars++;
    ...
    double speedup = (double)num_threads / total_time;
